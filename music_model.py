...
class MusicPredictionModel(nn.Module):
    def __init__(self, num_continuous, category_offsets, hidden_size, num_layers, embedding_dim, device):
        ...
        self.combined_linear = nn.Linear(152, 64)
        ...
        self.head_categorical = nn.Linear(192, self.num_categories, bias=False)

        self.inst_norm_1 = nn.LayerNorm(64, elementwise_affine=True)
        self.inst_norm_2 = nn.LayerNorm(64, elementwise_affine=True)
        self.inst_norm_3 = nn.LayerNorm(192, elementwise_affine=True)

        self.categorical_ln = nn.LayerNorm(192, elementwise_affine=True)
        self.continuous_ln = nn.LayerNorm(192, elementwise_affine=True)
    ...
    def forward(self, x_categorical, x_continuous, sequence_lengths, hidden_state):
        ...
        x_categorical_embedded = self.embeddings(x_categorical)
        x_categorical_embedded = x_categorical_embedded.view(batch_size, seq_len, -1)
        ...
        # split up the categorical vars
        octave = cat_output[:, 0:self.category_offsets[0]]
        noteName = cat_output[:, self.category_offsets[0]:]
        ...
        cat_output = torch.cat([octave, noteName], dim=1)
